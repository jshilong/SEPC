...
from sepc.self_mmdet.core.fsaf_anchor_target import fsaf_anchor_target, multi_apply
...
class FSAFHead(RetinaHead):
    ...
    def forward_single(self, x):
        ...
        return cls_score, self.relu(bbox_pred)  # TBLR encoder only accepts positive bbox_pred
    ...
    def loss(self,
        ...
        return dict(loss_cls=losses_cls, loss_bbox=losses_bbox, num_pos=num_pos / batch_size, accuracy=accuracy)
    ...
    def collect_loss_level_single(self, cls_loss, reg_loss, pos_assigned_gt_inds, labels_seq):
        ...
        loss_ceiling = loss.new_zeros(1).squeeze() + 1e6  # default loss value for a layer where no anchor is positive
        losses_ = torch.stack(
    ...
    def reassign_loss_single(self, cls_loss, reg_loss, pos_assigned_gt_inds, labels, level, min_levels):
        ...
        unmatch_gt_inds = torch.nonzero(min_levels != level)  # gts indices that unmatch with the current level
