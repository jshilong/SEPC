...
def is_located_in(points, bboxes, is_aligned=False):
    ...
    Args:
      ...
      bboxes: (tensor)  bounding boxes, Shape: [n,4].  If is_aligned is ``True``, then m mush be equal to n
    Return:
      (tensor) intersection area, Shape: [m, n]. If is_aligned is ``True``, shape = [m]
    """
    if not is_aligned:
...
class EffectiveAreaAssigner(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None):
        ...
        is_bbox_in_gt = is_located_in(bbox_centers, gt_bboxes)  # the center points lie within the gt boxes
        bbox_and_gt_eff_overlaps = bbox_overlaps(bboxes, gt_eff, mode='iof')
        is_bbox_in_gt_eff = is_bbox_in_gt & (bbox_and_gt_eff_overlaps > self.min_pos_iof)  # shape (n, k)
        ...
        assign_result = self.assign_wrt_areas(is_bbox_in_gt_eff, is_bbox_in_gt_ignore, gt_labels, gt_priority=sort_idx)
        return assign_result

    def assign_wrt_areas(self, is_bbox_in_gt_eff, is_bbox_in_gt_ignore, gt_labels=None, gt_priority=None):
        ...
        bbox_priority = is_bbox_in_gt_eff.new_full((num_bboxes, num_gts),
                                                   -1,
                                                   dtype=torch.long)
        inds_of_match = torch.any(is_bbox_in_gt_eff, dim=1)  # whether the bbox is matched (to any gt)

        # Each bbox could match with multiple gts. The following codes deals with this
